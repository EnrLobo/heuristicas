...
class VNSResult:
    ...
    history_x: List[List[int]] = field(default=None, repr=False)  # solucao apos cada iteracao (so quando track_solutions=True)
    iterations: int = 0
...
def vns(
    ...
    track_solutions=True faz o algoritmo tambem guardar uma copia do vetor
    solucao a cada iteracao (usado pela API do site para animar a mochila
    enchendo em tempo real). Custa mais memoria/tempo, entao fica desligado
    por padrao (ex: quando rodamos a bateria de 486 experimentos, so o
    fitness de cada iteracao importa).
    """
    ...
    history_x = [list(x)] if track_solutions else None
